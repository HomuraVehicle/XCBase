...
namespace xc{
	...
	public:
		...
		chain(rvalue_reference<this_type>& rref){
			...
			Size = rref.ref.Size;
			rref.ref.Size = 0;
			base::splice_range_element(*(rref.ref.Sentinel.next), rref.ref.Sentinel,Sentinel);
		}
	...
	public:
		...
		T& front(){ return *static_cast<T*>(Sentinel.next); }
		T& back(){ return *static_cast<T*>(Sentinel.prev); }
		const T& front()const{ return *static_cast<const T*>(Sentinel.next); }
		const T& back()const{ return *static_cast<const T*>(Sentinel.prev); }
		...
		void pop_back(){
			...
			--Size;
			base::erase_element(*(Sentinel.prev));
		}
		void push_front(T& Elem){
			++Size;
			base::insert_element(Elem, *(Sentinel.next));
		}
		void pop_front(){
			if(empty())return;

			--Size;
			base::erase_element(*(Sentinel.next));
		}
		...
		void splice(const_iterator pos, this_type& other){
			...
			base::splice_range_element(*(other.Sentinel.next), other.Sentinel, *const_cast<element*>(pos.current()));
		}
